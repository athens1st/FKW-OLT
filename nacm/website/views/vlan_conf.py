	...
	def post(self, request, *args, **kwargs):
		...
		if ipform.is_valid() and formm.is_valid():
			...
			for form in ipform:
				...
				ipport = form.cleaned_data.get('ipport')
				vendor = form.cleaned_data.get('vendor')
				
				dba_profile = (request.POST.getlist('dba_profile'))
				mode_dba_profile = (request.POST.getlist('mode_dba_profile'))
				sla_fixed = (request.POST.getlist('sla_fixed'))
				sla_assured = (request.POST.getlist('sla_assured'))
				sla_maximum = (request.POST.getlist('sla_maximum'))


				# interface = str(request.POST['interface'])
				collect_config = "<b>Configure on "+str(ipaddr)+" | Port "+str(ipport)+" | vendor = "+str(vendor)+"</b></br>"
				print (vendor)
				...
				try:
					...
					ssh_client.connect(hostname=ipaddr,port=ipport,username=userValue,password=passValue) #,look_for_keys=False, allow_agent=False, timeout=5)
					...
					for x in range(len(dba_profile)):
						dba_profile = dba_profile[x]
						mode_dba_profile = mode_dba_profile[x]
						sla_fixed = sla_fixed[x]
						sla_assured = sla_assured[x]
						sla_maximum = sla_maximum[x]
							...
							if new_line != '':
								...
								try:
									...
									try:
										...
										time.sleep(1)
										results = remote_conn.recv(60000)
